...
import { MONTHS, SALES_QUALITY_SHEET_URL, SALES_CLAIMS_SHEET_URL, CEM_OS_SHEET_URL, CEM_OS_SALTA_SHEET_URL } from '../constants';
...
const SalesQualityDashboard: React.FC<SalesQualityDashboardProps> = ({ onBack, initialTab = 'surveys', config }) => {
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        // Load Surveys
        const sData = await fetchSalesQualityData(config.sheetUrls.sales_quality || SALES_QUALITY_SHEET_URL);
        setSurveyData(sData);
        setAvailableSurveyBranches([...new Set(sData.map(d => d.sucursal))].filter(Boolean).sort());
        
        // Load Claims
        const cData = await fetchSalesClaimsData(config.sheetUrls.sales_claims || SALES_CLAIMS_SHEET_URL);
        setClaimsData(cData);
        ...
        const [cemJujuy, cemSalta] = await Promise.all([
          fetchCemOsData(config.sheetUrls.cem_os || CEM_OS_SHEET_URL),
          fetchCemOsData(config.sheetUrls.cem_os_salta || CEM_OS_SALTA_SHEET_URL)
        ]);
