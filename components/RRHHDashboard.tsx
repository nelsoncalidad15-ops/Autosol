...
const RRHHDashboard: React.FC<RRHHDashboardProps> = ({ gradesUrl, relatorioUrl, onBack }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      console.log("RRHHDashboard: Starting data load...");
      console.log("RRHHDashboard: gradesUrl =", gradesUrl);
      console.log("RRHHDashboard: relatorioUrl =", relatorioUrl);
      ...
      try {
        ...
        console.log("RRHHDashboard: Grades count =", gradesData.length);
        console.log("RRHHDashboard: Relatorio count =", relatorioData.length);
        ...
        <div className="flex-1 p-6 max-w-[1600px] mx-auto w-full">
          {loadingState === LoadingState.LOADING ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400">
              ...
              <p className="text-lg font-bold">Error al cargar los datos de RRHH</p>
              <button 
                onClick={() => window.location.reload()}
                className="mt-4 px-6 py-2 bg-[#001E50] text-white rounded-xl font-bold"
              >
                Reintentar
              </button>
            </div>
