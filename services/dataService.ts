...
export const fetchSalesQualityData = async (csvUrl: string): Promise<SalesQualityRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchSalesClaimsData = async (csvUrl: string): Promise<SalesClaimsRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchDetailedQualityData = async (csvUrl: string): Promise<DetailedQualityRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchPostventaKpiData = async (csvUrl: string): Promise<PostventaKpiRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchPostventaBillingData = async (csvUrl: string): Promise<BillingRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchPCGCData = async (csvUrl: string): Promise<PCGCRecord[]> => {
    try {
      const response = await fetch(csvUrl);
      if (!response.ok) throw new Error(`Failed to fetch data: ${response.statusText}`);
      const text = await response.text();
...
export const fetchHRGradesData = async (csvUrl: string): Promise<CourseGrade[]> => {
    if (!csvUrl) {
        console.warn("fetchHRGradesData: No CSV URL provided");
        return [];
    }
    try {
        const fetchUrl = csvUrl.includes('?') ? `${csvUrl}&t=${Date.now()}` : `${csvUrl}?t=${Date.now()}`;
        console.log("fetchHRGradesData: Fetching from", fetchUrl);
        const response = await fetch(fetchUrl);
        if (!response.ok) {
            console.error(`fetchHRGradesData: Failed to fetch (Status: ${response.status} ${response.statusText})`);
            throw new Error(`Failed to fetch HR grades: ${response.statusText}`);
        }
        const text = await response.text();
        console.log(`fetchHRGradesData: Received ${text.length} characters`);
        if (text.length < 100) {
            console.warn("fetchHRGradesData: Received very short response, might be empty or error page");
            console.log("fetchHRGradesData: Content preview:", text.substring(0, 200));
        }
        const data = parseHRGradesCSV(text);
        console.log(`fetchHRGradesData: Parsed ${data.length} records`);
        return data;
    } catch (error) {
        console.error("Error loading HR grades data from:", csvUrl, error);
        return [];
    }
};

export const fetchHRRelatorioData = async (csvUrl: string): Promise<RelatorioItem[]> => {
    if (!csvUrl) {
        console.warn("fetchHRRelatorioData: No CSV URL provided");
        return [];
    }
    try {
        const fetchUrl = csvUrl.includes('?') ? `${csvUrl}&t=${Date.now()}` : `${csvUrl}?t=${Date.now()}`;
        console.log("fetchHRRelatorioData: Fetching from", fetchUrl);
        const response = await fetch(fetchUrl);
        if (!response.ok) {
            console.error(`fetchHRRelatorioData: Failed to fetch (Status: ${response.status} ${response.statusText})`);
            throw new Error(`Failed to fetch HR relatorio: ${response.statusText}`);
        }
        const text = await response.text();
        console.log(`fetchHRRelatorioData: Received ${text.length} characters`);
        if (text.length < 100) {
            console.warn("fetchHRRelatorioData: Received very short response, might be empty or error page");
            console.log("fetchHRRelatorioData: Content preview:", text.substring(0, 200));
        }
        const data = parseHRRelatorioCSV(text);
        console.log(`fetchHRRelatorioData: Parsed ${data.length} records`);
        return data;
    } catch (error) {
        console.error("Error loading HR relatorio data from:", csvUrl, error);
        return [];
    }
};
...
const parseDetailedQualityCSV = (csvText: string): DetailedQualityRecord[] => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      headers.forEach((header, index) => {
        ...
        if (header === 'mes') record.mes = normalizeMonth(value);
        else if (header === 'codid') record.cod_id = value.trim();
        else if (header === 'fecha de servicio') record.fecha_servicio = value;
        else if (header === 'vin') record.vin = value;
        else if (header === 'modelo') record.modelo = value;
        else if (header === 'or') record.orden = value;
        else if (header === 'asesor') record.asesor = cleanAsesorName(value);
    ...
    // Filter out records without a valid CodID
    return records.filter(r => r.cod_id && r.cod_id.trim() !== "" && r.cod_id !== "0");
};
...
const parsePostventaKpiCSV = (csvText: string): PostventaKpiRecord[] => {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        headers.forEach((header, index) => {
            ...
            if (header === 'sucursal') record.sucursal = normalizeBranch(value);
            else if (header === 'mes') record.mes = normalizeMonth(value);
            else if (header.includes('anio') || header.includes('ano')) record.anio = parseInt(value) || 2026;
        ...
        if (record.sucursal && record.mes && record.mes !== 'Unknown') {
...
const parsePostventaBillingCSV = (csvText: string): BillingRecord[] => {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        headers.forEach((header, index) => {
            ...
            if (header === 'nro mes') record.nro_mes = parseInt(value) || 0;
            else if (header === 'mes') record.mes = normalizeMonth(value);
            else if (header.includes('anio') || header.includes('ano')) record.anio = parseInt(value) || 2026;
            else if (header === 'sucursal') record.sucursal = normalizeBranch(value);
            else if (header === 'area') record.area = value;
            else if (header.includes('objetivo mensual')) record.objetivo_mensual = parseNumber(value);
            else if (header.includes('avance a fecha')) record.avance_fecha = parseNumber(value);
            else if (header.includes('cumplimiento a la fecha')) record.cumplimiento_fecha_pct = parseNumber(value);
            else if (header.includes('cumplimiento a cierre mes')) record.cumplimiento_cierre_pct = parseNumber(value);
            else if (header.includes('objetivo diario')) record.objetivo_diario = parseNumber(value);
            else if (header.includes('prom. diario')) record.promedio_diario = parseNumber(value);
            else if (header.includes('desvio a fecha')) record.desvio_fecha = parseNumber(value);
...
export const fetchCemOsData = async (url: string): Promise<CemOsRecord[]> => {
    try {
...
export const fetchActionPlanData = async (url: string): Promise<ActionPlanRecord[]> => {
    try {
      const response = await fetch(url);
      const csvText = await response.text();
...
export const fetchInternalPostventaData = async (url: string): Promise<InternalPostventaRecord[]> => {
    try {
      const response = await fetch(url);
      const csvText = await response.text();
